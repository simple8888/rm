<?php
	...
	if(!$tpl->isCached("default/admin.tpl", $current_page)) {
     	        ...
     	        $sql2="SELECT * from admin  WHERE(1=1)  ";//日期是字符串类型，所以要加引号 
		...
		if(1) {                           	//如果从数据库中获取到商品记录
			$tpl->assign("tableName", "后台管理员表");  //分配页面显示的表面到模板中
			$tpl->assign("url", "admin.php");       //分配分页处理文件的URL到模板中
			$tpl->assign("admin", $admin);    //将在本页显示的所有数据数组分配给模板
     ...
     $tpl->assign("title","后台管理员表");
     $tpl->display("../templates/admin/admin.tpl", $current_page);     //加载输出模板kaoqinxiaoshi.tpl，并指定页面缓存标号
