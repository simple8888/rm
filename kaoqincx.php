	...
	if(!$tpl->isCached("default/kaoqincx.tpl", $current_page)) {
		...
		if($kaoqin) {                           	//如果从数据库中获取到商品记录
			...
			echo"<center><span><p style='margin-top:360px;'>*温馨提示*</p></span></center>";//输出一行提示信息
                        echo"<center>员工考勤&请假休假&福利&代扣款信息表中查无此人!</center>";echo"<br />";
                        echo"<center><input class='button'type='button' value='返回' onClick=window.location.href='index.php?p=kaoqin' ></center>";
                        exit;                              	//并退出程序
		}
	}
     $tpl->assign("biaoti","信息发布栏");
     $tpl->assign("pandianzhouqifabu",@$ar); 	
     $tpl->assign("title","考勤&请假休假&福利&代扣款查询");
