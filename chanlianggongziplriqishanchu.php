...
<?php
    ...
    if(!$riqi){
      ...
      $result=$mysqli->query("select riqi,ygongID,chanliangID from yuangong left join chanliang on yuangongID=ygongID where ($gzdd) and shifouzaizhi='是' ");
      @$i=0;
      while($row=$result->fetch_row()){
      $res=$mysqli->query("select count(ygongID) as YGID from chanliang where ygongID=$row[1]");
      $arr=$res->fetch_assoc(); 
      if((($arr["YGID"])>1) and ($riqi==$row[0])){ //删除ygongID数大于1的当日的数据，留最后一个 
      $query="delete from chanliang where riqi='$riqi' and chanliangID=$row[2]";
      $mysqli->query($query);}

      if((($arr["YGID"])==1) and ($riqi==$row[0])){//把留一下的最后一条记录，除了ygongID外全部清空，相当于删除该条记录，留下ygongID用来下次批量插入日期
      $mysqli->query("UPDATE chanliang SET gongxu='',riqi=null,chanliang='',channengbt='',zuidibt='',zhuanghuo='',xiehuo='',zahuo='',yichang='',chanlianggongzi='' WHERE ygongID=$row[1]");
                              }
      if($riqi==$row[0]){$i=$i+1;}
        }
