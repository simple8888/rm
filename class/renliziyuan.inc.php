<?php
require('salary.inc.header.html');
require('renliziyuandangan.inc.header.html');
?>
<html>
<head>
...
<?php
    ...
    $ID="<a href='".$_SERVER['PHP_SELF']."?p=id'>ID</a>";//.$_SERVER['PHP_SELF'].
    $table->setHeaderContents(0,0,$ID);//设置表头，循环处理数组以生成表数据
    
    $number="<a href='".$_SERVER['PHP_SELF']."?p=number'>工号</a>";
    $table->setHeaderContents(0,1,$number);
    ...
    $p=(isset($_GET['sort']))?$_GET['sort']:"id";
    
    $query="select id,number,name,sex,birthday,format((timestampdiff(day,birthday,now())/365),2),idno,cell,tel,email,marriage,hometown,livesite,feature,memo FROM yuangong ORDER BY ? DESC";
