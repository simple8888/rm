<?php
	...
	if(!$tpl->isCached("default/chanlianggongzi.tpl", $current_page)) {
                ...
                if(@$_SESSION[fac]!=null){
     	        ...
     	        $sql2="SELECT * from gongxu  WHERE(1=1) "; 
                $sql2.=@$ft;
         	...
         	/* 通过调用MyDB中的方法，获取当前页面所需要的所有记录的数据数组 */
                ...
                if(@$_SESSION[fac]!=null){
     	        ...
     	        $sql2="SELECT * from gongxu  WHERE(1=1)  "; 
                $s="order by gongxuID DESC";
                $sql2.=@$ft; 
                $sql2.=@$s;  
 
                $chanlianggongxu=$mydb->getPageRows($sql2);
		if(1) {                           	//如果从数据库中获取到商品记录
			$tpl->assign("tableName", "工序信息表");  //分配页面显示的表面到模板中
			$tpl->assign("url", "chanlianggongxu.php");       //分配分页处理文件的URL到模板中
			$tpl->assign("chanlianggongxu", $chanlianggongxu);    //将在本页显示的所有数据数组分配给模板
     ...
     $tpl->assign("title","工序表");
     $tpl->display("default/chanlianggongxu.tpl", $current_page);     //加载输出模板kaoqin.tpl，并指定页面缓存标号
