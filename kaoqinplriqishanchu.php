...
<?php
    ...
    if(!$riqi){
      ...
      $result=$mysqli->query("select riqi,ygongID,kaoqinID from yuangong left join kaoqin on yuangongID=ygongID where ($gzdd) and shifouzaizhi='是' ");
      @$i=0;
      while($row=$result->fetch_row()){
      $res=$mysqli->query("select count(ygongID) as YGID from kaoqin where ygongID=$row[1]");
      $arr=$res->fetch_assoc(); 
      if((($arr["YGID"])>1) and ($riqi==$row[0])){ //删除ygongID数大于1的当日的数据，留最后一个 
      $query="delete from kaoqin where riqi='$riqi' and kaoqinID=$row[2]";
      $mysqli->query($query);}

      if((($arr["YGID"])==1) and ($riqi==$row[0])){//把留一下的最后一条记录，除了ygongID外全部清空，相当于删除该条记录，留下ygongID用来下次批量插入日期
      $mysqli->query("UPDATE kaoqin SET riqi=null,fadingcq='',fadingjb='',zhoumojb='',guodingdx='',hunjia='',chanjia='',sangjia='',bingjia='',pingshiqj='',zhoumoqj='',quanqin='',dizeng='',peixun='',jintie='',hongbao='',fazhan='',gaowen='',gongshang='',shuidian='' WHERE ygongID=$row[1]");
                              }
      if($riqi==$row[0]){$i=$i+1;}
        }
