<?php
	class MyDBKQcxnd{
		...
		public function getRowTotal(){       		//调用该方法返回员工表yuangong中记录总数
			$result=$this->mysqli->query("select yuangong.yuangongID,kaoqin.kaoqinID from yuangong left join kaoqin ON yuangong.yuangongID=kaoqin.ygongID WHERE yuangong.shifouzaizhi='是'  ");  //执行Select语句
			return $result->num_rows;	                        //返回结果集中的记录总数
		}
		public function getPageRows($offset, $num){   	//获取指定一段的记录
			$kaoqin=array();
                        $query="SELECT yuangong.yuangongID,kaoqin.kaoqinID,kaoqin.ygongID,yuangong.number,yuangong.name,kaoqin.riqi,kaoqin.fadingcq,kaoqin.fadingjb,kaoqin.zhoumojb,kaoqin.guodingdx,kaoqin.hunjia,kaoqin.chanjia,kaoqin.sangjia,kaoqin.bingjia,kaoqin.pingshiqj,kaoqin.zhoumoqj,kaoqin.quanqin,kaoqin.dizeng,kaoqin.peixun,kaoqin.jintie,kaoqin.hongbao,kaoqin.fazhan,kaoqin.gaowen,kaoqin.gongshang,kaoqin.shuidian FROM yuangong left join kaoqin ON yuangong.yuangongID=kaoqin.ygongID WHERE yuangong.shifouzaizhi='是' ORDER BY kaoqin.ygongID DESC,kaoqin.riqi DESC LIMIT $offset, $num";
