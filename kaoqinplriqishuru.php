...
<?php
    ...
    if(!$riqi){
     ...
     //$result=$mysqli->query("select ygongID from  kaoqin ");//判断kaoqin表里的日期删完同时ygongID删除完时，向kaoqin表插入ygongID
...
//echo"$res->num_rows";
     //if((($result->num_rows)==0)or(($result->num_rows)===($res->num_rows))){
      ...
      //向kaoqin表批量插入日期,空表必须按yuangongID排序后才能批量插入  
      $result=$mysqli->query("select ygongID from yuangong left join kaoqin on yuangongID=ygongID  where ($gzdd) and ($sfzc) and shifouzaizhi='是'  group by ygongID");
      while($row=$result->fetch_row()){
