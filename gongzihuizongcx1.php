...
order by ygongID DESC");
     ...
     $gc="gongchang='罗麦' or gongchang='博涵'";//输出时隔离工作地点
     if(@$_SESSION[fac]!=null){                 //从common.php传过来,只显示登录人所在工厂的数据,如果登录人所在工厂为空,显示全部数据  
        $gc="gongchang='$_SESSION[fac]'";}     
     @$result=$mysqli->query("SELECT concat(gongchang,pandianyuefen) as gongpan FROM zhouqi where $gc ");
...
//echo"$factory<br />";
     ...
     if(($xiaoshizd=="")||($rizuidi=="")){      
      echo"<center><span><p style='margin-top:360px;'>*温馨提示*</p></span></center>";
      echo"<center>你所选的盘点周期对应的年度小时最低工资或日最低工资有一项或全部为空,补充完整后才能汇总工资!</center>";echo"<br />";
	...
	if(!$tpl->isCached("default/kaoqincx.tpl", $current_page)) {
     ...
     $tpl->assign("title","工资汇总查询");
     $tpl->display("default/gongzihuizongcx.tpl", $current_page);     //加载输出模板kaoqin.tpl，并指定页面缓存标号
