...
<?php
	...
	if(!$tpl->isCached("default/kaoqincx.tpl", $current_page)) {
		...
		if($zhilianggongzi) {                           	//如果从数据库中获取到商品记录
			$tpl->assign("tableName", "员工质量工资信息查询表");  //分配页面显示的表面到模板中
			$tpl->assign("url", "zhilianggongzicx.php");       //分配分页处理文件的URL到模板中
     ...
     $tpl->assign("title","质量工资查询");
     $tpl->display("default/zhilianggongzicx.tpl", $current_page);     //加载输出模板kaoqin.tpl，并指定页面缓存标号
