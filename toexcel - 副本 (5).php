...
$objPHPExcel->setActiveSheetIndex(0)
	    ...
	    ->setCellValue("g".$j, '国定待薪')
            ...
            ->setCellValue("ag".$j, '应纳税额')
            ->setCellValue("ah".$j, '个税')
            ->setCellValue("ai".$j, '实发工资')
	    ...
	    ->setCellValue("f".$i, $rs[5])//周末加班
            ...
            ->setCellValue("x".$i, "=if((D$i*$xiaoshizd+E$i*$xiaoshizd*1.5+F$i*$xiaoshizd*2+AF$i)>(N$i+O$i+P$i+Q$i+W$i+Y$i+Z$i+AA$i+AB$i+AC$i+AD$i),(D$i*$xiaoshizd+E$i*$xiaoshizd*1.5+F$i*$xiaoshizd*2+AF$i)-(N$i+O$i+P$i+Q$i+W$i+Y$i+Z$i+AA$i+AB$i+AC$i+AD$i),0)")//小时最低工资补贴 需计算 如果要显示"",需用转义符\"\"
            ...
            ->setCellValue("ae".$i, "=round((G$i+H$i*$rizuidi*0.8+I$i*$rizuidi*0.8+J$i*$rizuidi*0.8+K$i*$rizuidi*0.8+N$i+O$i+P$i+Q$i+R$i+S$i+T$i+U$i+W$i+X$i+Y$i+Z$i+AA$i+AB$i+AC$i+AD$i),2)") //应发工资 需计算
            ->setCellValue("af".$i, $rs[30]) //社保
            ->setCellValue("ag".$i, "=if((AE$i-AF$i-3500)<0,0,(AE$i-AF$i-3500))") //应纳税额 系统里没有这项
            ->setCellValue("ah".$i, "=round((if(AG$i<=1500,AG$i*0.03,if(AG$i<=4500,AG$i*0.1-105,if(AG$i<=9000,AG$i*0.2-555,if(AG$i<=35000,AG$i*0.25-1005,if(AG$i<=55000,AG$i*0.3-2755,if(AG$i<=80000,AG$i*0.35-5505,AG$i0.45-13505))))))),2)") //个税 需计算
            ->setCellValue("ai".$i, "=if((AE$i-AF$i-AH$i-V$i)<=0,\"\",round((AE$i-AF$i-AH$i-V$i),2))");//实发工资 需计算 
$objPHPExcel->getActiveSheet()->getStyle('A2:AI'.$i)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);//边框
$objPHPExcel->getActiveSheet()->getStyle('A2:AI'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);//水平居中
$objPHPExcel->getActiveSheet()->getStyle('A2:AI'.$i)->getFont()->setSize(9);//字体大小
