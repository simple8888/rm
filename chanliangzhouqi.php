<?php
	...
	if(!$tpl->isCached("default/chanlianggongzi.tpl", $current_page)) {
		...
		if(1) {                           	//如果从数据库中获取到商品记录
			$tpl->assign("tableName", "盘点周期信息表");  //分配页面显示的表面到模板中
			$tpl->assign("url", "chanliangzhouqi.php");       //分配分页处理文件的URL到模板中
			$tpl->assign("chanliangzhouqi", $chanliangzhouqi);    //将在本页显示的所有数据数组分配给模板
     ...
     $tpl->assign("title","盘点周期表");
     $tpl->display("default/chanliangzhouqi.tpl", $current_page);     //加载输出模板chanliangzhouqi.tpl，并指定页面缓存标号
