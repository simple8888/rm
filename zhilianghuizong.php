...
<?php
	...
	if(!$tpl->isCached("default/kaoqincx.tpl", $current_page)) {
                ...
                if(@$_SESSION[fac]!=null){
     	        ...
     	        $sql2="SELECT yuangong.yuangongID,zhiliang.zhiliangID,zhiliang.ygongID,yuangong.number,yuangong.name,zhiliang.riqi,sum(zhiliangjl) as zhiliangjl,sum(zhiliangkk) as zhiliangkk,sum(zhilianggongzi) as zhilianggongzi FROM yuangong left join zhiliang ON yuangong.yuangongID=zhiliang.ygongID  WHERE(1=1)  ";//日期是字符串类型，所以要加引号 
                $s="and yuangong.shifouzaizhi='是' group by ygongID "; 
         	...
         	/* 通过调用MyDB中的方法，获取当前页面所需要的所有记录的数据数组 */
                ...
                if(($qishirq!=null)and($zhongzhirq!=null)){   
                    $rq="and ((zhiliang.riqi between ('$qishirq') and ('$zhongzhirq'))  or zhiliang.riqi is null) and yuangong.shifouzaizhi='是'  ";}
                if($number!=null){
                ...
                $s="and yuangong.shifouzaizhi='是' group by ygongID ORDER BY zhiliang.ygongID DESC,zhiliang.riqi DESC LIMIT $offset, $num";//空表必须按yuangongID排序后才能批量插入  
                $sql2.=@$rq; 
                $sql2.=@$nb; 
                $sql2.=@$nm; 
                $sql2.=@$ft;
		...
		if($zhilianggongzi) {                           	//如果从数据库中获取到商品记录
			$tpl->assign("tableName", "员工质量工资汇总表");  //分配页面显示的表面到模板中
			$tpl->assign("url", "zhilianghuizong.php");       //分配分页处理文件的URL到模板中
			...
			echo"<center><span><p style='margin-top:360px;'>*温馨提示*</p></span></center>";//输出一行提示信息
                        echo"<center>员工质量工资信息表中查无此人!</center>";echo"<br />";
                        echo"<center><input class='button'type='button' value='返回' onClick=window.location.href='index.php?p=zhilianggongzi' ></center>";
                        exit;                              	//并退出程序
		}
	}
	
     $tpl->assign("title","质量工资汇总");
     $tpl->display("default/zhilianghuizong.tpl", $current_page);     //加载输出模板kaoqin.tpl，并指定页面缓存标号
